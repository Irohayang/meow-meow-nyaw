...
IFS=$'\n'
wallpapers=($(find /home/iroha/Pictures/sex -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) | shuf))

while true; do
    for wp in "${wallpapers[@]}"; do
        echo "🖼️ Đang đổi sang: $wp"
        if feh --bg-scale --zoom fill "$wp"; then
            echo "✅ Thành công"
        else
            echo "❌ Lỗi ảnh, bỏ qua"
        fi
        sleep 5 # Change every 10 minutes
    done
    wallpapers=($(find /home/iroha/Pictures/sex -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) | shuf))  # Reshuffle after full loop
done
